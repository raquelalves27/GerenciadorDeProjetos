...
import type { ProjectStatus, ProjectPriority, ShiftType, AlertType, TaskStatus, TaskImpact } from '@/types'
...
export const TASK_STATUS_CONFIG: Record<TaskStatus, { label: string; color: string; dot: string }> = {
  a_fazer: { label: 'A fazer', color: 'bg-gray-100 text-gray-700 border-gray-200', dot: 'bg-gray-400' },
  em_andamento: { label: 'Em andamento', color: 'bg-blue-50 text-blue-700 border-blue-200', dot: 'bg-blue-500' },
  aguardando_terceiro: { label: 'Aguardando terceiro', color: 'bg-amber-50 text-amber-700 border-amber-200', dot: 'bg-amber-500' },
  atrasado: { label: 'Atrasado', color: 'bg-red-50 text-red-700 border-red-200', dot: 'bg-red-500' },
  monitoramento: { label: 'Monitoramento', color: 'bg-purple-50 text-purple-700 border-purple-200', dot: 'bg-purple-500' },
  concluido: { label: 'Concluído', color: 'bg-green-50 text-green-700 border-green-200', dot: 'bg-green-500' },
}

export const TASK_IMPACT_CONFIG: Record<TaskImpact, { label: string; color: string; bar: string }> = {
  baixo: { label: 'Baixo', color: 'bg-slate-100 text-slate-600', bar: 'bg-slate-400' },
  medio: { label: 'Médio', color: 'bg-blue-50 text-blue-700', bar: 'bg-blue-500' },
  alto: { label: 'Alto', color: 'bg-amber-50 text-amber-700', bar: 'bg-amber-500' },
  critico: { label: 'Crítico', color: 'bg-red-50 text-red-700', bar: 'bg-red-500' },
}
